...
function Courses() {
  ...
  const handleRate = async (courseId, rating) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Failed to rate course');
    } finally {
  ...
  return (
    <div className="courses-page" style={{ maxWidth: '1400px', margin: '2rem auto', padding: '0 clamp(1rem, 3vw, 2rem)' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: 'clamp(1.5rem, 4vw, 2rem)', color: 'var(--text-primary)', marginBottom: '0.5rem' }}>
          Explore Courses
        </h2>
        <p style={{ color: 'var(--text-tertiary)' }}>
          Discover {courses.length}+ courses across all topics
        </p>
      </div>
          ...
          {filteredCourses.map(course => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
                ...
                <span style={{
                  padding: '0.25rem 0.75rem',
                  background: course.price ? '#fbbf2420' : '#34d39920',
                  color: course.price ? '#f59e0b' : '#34d399',
                  borderRadius: '20px',
                  fontSize: '0.75rem',
                  fontWeight: '600',
                  marginLeft: 'auto',
